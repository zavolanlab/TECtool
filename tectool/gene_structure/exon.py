...
class Exon:
    ...
    """This class represents an Exon
        ...
        :param transcript_id: unique id for the transcript to which the exon belongs to (e.g. Ensembl Transcript ID).
        :param gene_id: unique id for the gene to which the exon belongs to (e.g. Ensembl Gene ID).
    ...
    *Class members*
        ...
        *gene_id*
            String. Unique id for the gene to which the exon belongs to (e.g. Ensembl Gene ID).

        *transcript_id*
            String. Unique id for the transcript to which the exon belongs to (e.g. Ensembl Transcript ID).
        ...
        *start_codon*
            HTSeq Genomic interval. Start codon region. None in case of no start codon.

        *stop_codon*
            HTSeq Genomic interval. Stop codon region. None in case of no stop codon.

    """

    def __init__(self, chromosome, source, feature, start, end, score, strand, frame, gene_id, transcript_id, exon_number, gene_name, gene_biotype, transcript_name, transcript_biotype, exon_id):
    ...
    def has_equal_coordinates(self, exon):
        ...
        """ 
        Method that checks whether the exon has equal coordinates to a given exon.
        """
    ...
    def to_bed(self):
        ...
        return("\t".join([self.chromosome, str(self.start), str(self.end), self.gene_id, ".", self.strand+"\n"]))


    def write_exon_gtf(self, output_file, with_CDS):


        """ Write exon in gtf format. If with_CDS is specified then the start codons, CDSs, and stop codons are also written. """
